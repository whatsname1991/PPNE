...
// Modified from Google Word2Vec toolkit and LiuQuan SWE toolkit.
...
real PPNE_hinge_margin = 0.0; // Hinge function low bound, f(x) = max(low_bound, x)
...
struct TypeQppneVal KnowDB_QppneVal; // running curve trends
struct TypeQppneVal KnowDB_QppneVal_CV; // running curve trends
...
{    
  ...
  {
	KnowDB_TermKDB[i].KDB_IDset = (long long*)calloc(PPNE_KDBNUM, sizeof(long long));
  ...
  /// read inequation num
  /// file format: sim(node_A, node_B) > sim(node_C, node_D)
  while (1) 
  {
  ...
  fclose(fKNOW);  
  printf("--- Finish reading the Constraints\n");
}
void PPNE_AddToKnowDB_InSet(int word_index, long long eleID)
{ 
	int kdb_num = 0;
...
{    
	...
	for (i = 0; i < vocab_size; ++i)
	{
		KnowDB_TermKDB_CV[i].KDB_IDset = (long long*)calloc(PPNE_KDBNUM, sizeof(long long));
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
	...
	{ 
	  /////////////////////////////////
	  ///  Add inequation constraint  ///
	  /////////////////////////////////
		if (PPNE_inter_coeff > 0.0 && (run_process > PPNE_add_time && KnowDB_TermKDB[word].KDB_nums >= 1))
		{
			PPNE_QppneDerive_Cosine(word, derive_distEu);												
			
			PPNE_VectorScale(derive_distEu, (-1.0*PPNE_inter_coeff * alpha));
			
			// weight deacy
			if (PPNE_weight_decay > 0.0) 
			{
				PPNE_VectorCopy(vector_decay, &syn0[word * layer1_size]);
				PPNE_VectorLinear(&syn0[word * layer1_size], vector_decay, (-1.0*alpha*PPNE_weight_decay));
			}
			PPNE_VectorLinear(&syn0[word * layer1_size], derive_distEu, 1.0);
		}
...
int main(void) {
	...
	if (argc == 1) {
	printf("PPNE: Property Preserving Toolkit\n");
	...
	printf("\t\tSet the constraint training set to <file>\n");
	printf("\t-ppne-valid <file>\n");
	printf("\t\tSet the constraint validation set to <file>\n");
	...
	printf("\t\tSet the time (in process: %) for adding constraint. default is 0 (add at the begining)\n");
	printf("\t-delta-left <int>\n");
	printf("\t\tUse the left node pair of each similarity inequality or not. default is 1\n");
	printf("\t-delta-right <int>\n");
	printf("\t\tUse the right node pair of each similarity inequality or not. default is 1\n");
	...
	layer1_size = 160;
	strcpy(train_file, "citeseer.walks.txt");
	strcpy(output_file, "citeseer.embedding.ppne.txt");
	window = 3;
	negative = 5;
	hs = 0;
	binary = 0;
	cbow = 0;
	iter = 1;
	strcpy(PPNE_inequation_file, "train.citeseer.inequation.constraints.txt");
	strcpy(PPNE_inequation_fileCV, "valid.citeseer.inequation.constraints.txt");
	...
	printf("Property Preserving Embedding (PPNE) Toolkit");
